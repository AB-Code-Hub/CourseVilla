...
const EditCourse = () => {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        setLoading(true);
        const course = await getCourseByCourseId(id);
  ...
  return (
    <motion.div 
      ...
      className="px-4 sm:px-6 lg:px-8 py-8 max-w-4xl mx-auto"
    >
      ...
      <motion.div variants={itemVariants} className="bg-white shadow rounded-lg">
        <div className="px-6 py-5 border-b border-slate-200">
          <h2 className="text-xl font-bold text-slate-900">Edit Course</h2>
          <p className="mt-1 text-sm text-slate-500">Update the course details below</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {/* Form Actions */}
          <motion.div variants={itemVariants} className="flex justify-end space-x-3 pt-6 border-t border-slate-200">
            <button
              type="button"
              onClick={() => navigate(`/admin/courses/${id}`)}
