...
import { deleteCourseByCourseId } from '../../service/CourseService';

const DeleteCourse = ({ isOpen, onClose, courseId, courseTitle, onSuccess }) => {
  const [isDeleting, setIsDeleting] = useState(false);

  const handleDelete = async () => {
    try {
      setIsDeleting(true);
      const response  = await deleteCourseByCourseId(courseId);
      console.log("course response ", response);
      toast.success(`"${response.data?.data?.title}" deleted successfully`);
      onSuccess();
      onClose();
    } catch (error) {
      toast.error(error.response?.data?.message || `Failed to delete "${courseTitle}"`);
    } finally {
      setIsDeleting(false);
    }
  };
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        Are you sure you want to delete this course? This action cannot be undone. All course content, including sections and lessons, will be permanently removed.
                      </p>
                    </div>
                  </div>
                </div>
