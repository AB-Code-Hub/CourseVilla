...
const Navbar = () => {
  ...
  const adminLinks = (
    <>
      <Link
        to="/admin/dashboard"
        className={`px-3 py-2 text-sm font-medium ${location.pathname === '/admin/dashboard' ? 'text-blue-600' : 'text-slate-700 hover:text-blue-600'}`}
      >
        Dashboard
      </Link>
      <Link
        to="/admin/users"
        className={`px-3 py-2 text-sm font-medium ${location.pathname === '/admin/users' ? 'text-blue-600' : 'text-slate-700 hover:text-blue-600'}`}
      >
        Users
      </Link>
      <Link
        to="/admin/courses"
        className={`px-3 py-2 text-sm font-medium ${location.pathname === '/admin/courses' ? 'text-blue-600' : 'text-slate-700 hover:text-blue-600'}`}
      >
        Courses
      </Link>
    </>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex-shrink-0 flex items-center">
            <Link to={isAdminRoute ? "/admin" : "/"} className="flex items-center">
              <img
                className="mx-auto h-12 w-auto"
                src="data:image/svg+xml;base64,"
                alt="Company Logo"
              />
            ...
            <>
              {isAdminRoute ? (
                <>
                  <Link
                    to="/admin/dashboard"
                    className={`block px-3 py-2 rounded-md text-base font-medium ${location.pathname === '/admin/dashboard' ? 'text-blue-600 bg-blue-50' : 'text-slate-700 hover:text-blue-600 hover:bg-slate-100'}`}
                  >
                    Dashboard
                  </Link>
                  <Link
                    to="/admin/users"
                    className={`block px-3 py-2 rounded-md text-base font-medium ${location.pathname === '/admin/users' ? 'text-blue-600 bg-blue-50' : 'text-slate-700 hover:text-blue-600 hover:bg-slate-100'}`}
                  >
                    Users
                  </Link>
                  <Link
                    to="/admin/courses"
                    className={`block px-3 py-2 rounded-md text-base font-medium ${location.pathname === '/admin/courses' ? 'text-blue-600 bg-blue-50' : 'text-slate-700 hover:text-blue-600 hover:bg-slate-100'}`}
                  >
                    Courses
                  </Link>
                </>
